...
type RestOf<T extends ICategory | NewCategory> = Omit<T, 'createDt' | 'modifyDt'> & {
...
export class CategoryService {
  ...
  protected convertDateFromClient<T extends ICategory | NewCategory | PartialUpdateCategory>(category: T): RestOf<T> {
    return {
      ...category,
      createDt: category.createDt?.format(DATE_FORMAT) ?? null,
      modifyDt: category.modifyDt?.format(DATE_FORMAT) ?? null,
    };
  }
  ...
  protected convertDateFromServer(restCategory: RestCategory): ICategory {
    return {
      ...restCategory,
      createDt: restCategory.createDt ? dayjs(restCategory.createDt) : undefined,
      modifyDt: restCategory.modifyDt ? dayjs(restCategory.modifyDt) : undefined,
    };
  }
